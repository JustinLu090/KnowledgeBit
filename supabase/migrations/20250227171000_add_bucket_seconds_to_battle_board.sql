-- Allow configurable settlement bucket length (default 3600s).
-- This enables fast testing (e.g. 120s) while keeping production behavior unchanged.
...
BEGIN
  ...
  -- 2) Previous bucket board (or initial)
  ...
  -- 3) Allocations from previous bucket (client submits previous bucket at settlement time)
  ...
  LOOP
    FOR v_idx IN 0..15 LOOP
      v_others_alloc := jsonb_set(
        v_others_alloc,
        ARRAY[v_idx::text],
        to_jsonb((COALESCE((v_others_alloc->>v_idx::text)::int, 0) + COALESCE((v_other_rec.allocations->>v_idx::text)::int, 0))::text::int)
      );
    END LOOP;
  END LOOP;

  -- 4) Apply reinforce then decay + enemy pressure then attacks
  FOR v_idx IN 0..15 LOOP
