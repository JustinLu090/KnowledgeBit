-- 固定格子編號：左上 #1 = 紅隊(invited)、右下 #16 = 藍隊(creator)。
-- 後端改為儲存 creator/invited，回傳給前端時再依當前使用者轉成 player/enemy。
...
BEGIN
  ...
  -- 若前一 bucket 尚未寫入（例如 02:32 請求先於 02:30 寫入），先遞迴計算並寫入 02:30，避免 fallback 到初始盤面導致剛佔領的格子消失
  ...
  -- 舊資料可能存 player/enemy，轉成 creator/invited 以便後續邏輯一致
  ...
  -- 回傳給前端時轉成 player/enemy：左上 #1=紅隊 右下 #16=藍隊
  FOR v_idx IN 0..15 LOOP
