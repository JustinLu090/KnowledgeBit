-- 地圖盤面與統整一致：get_battle_board_state / compute_one_battle_step 改為用「時間範圍」讀取
-- battle_allocations 與 battle_board_state，避免 hour_bucket 精確比對對不到導致地圖不更新。

-- 1. compute_one_battle_step：讀取 allocations 改為範圍
...
-- 2. get_battle_board_state：讀取快取盤面與 allocations 改為範圍
