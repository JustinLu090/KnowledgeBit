...
KnowledgeBit 是為現代「碎片化時間」設計的單字卡學習工具。有別於需要主動打開的傳統 App，KnowledgeBit 透過 **WidgetKit** 將知識推送到主畫面；並以 **App Groups** 在主 App 與小工具間同步資料，形成完整學習迴圈：**小工具瀏覽 → App 測驗／複習 → 連續天數與經驗值獎勵**。登入後更可同步至 **Supabase**、與好友共編單字集、發起**戰略對戰**，並以 **AI（OpenAI）** 一鍵產生單字卡與選擇題。
...
| **AI 產生單字卡** | 在新增單字時可輸入主題，呼叫 Supabase Edge Function，使用 **OpenAI（gpt-4o-mini）** 一次產生多張單字卡（word / definition / example），並可排除單字集內已有單字，避免重複。 |
...
| AI 產生 | Supabase Edge Function + OpenAI API（gpt-4o-mini，含 Responses API 處理講義 PDF） |
| 圖表 | Swift Charts（成就頁） |
| 版本控制 | Git / GitHub |
...
│   └── functions/
│       ├── generate-card/          # 依主題用 OpenAI 產生多張單字卡
│       ├── generate-quiz/          # 依單字集卡片產生選擇題
│       ├── generate-from-lecture/  # 講義 PDF → 摘要 + 學習卡 + 測驗（OpenAI Responses API）
│       └── join-preview/           # 邀請連結網頁預覽頁
...
- **Supabase（使用新版 Publishable Key）**  
  在 [Supabase Dashboard](https://supabase.com/dashboard) → **Project Settings → API Keys** → 切到 **Publishable and secret API keys**，建立 **Publishable key**（格式 `sb_publishable_...`）。此 key 供 client（iOS App）使用，受 RLS 保護；**勿使用 secret key**。本專案為 iOS App，唯一 client 即 App 本身，因此不需要在 App 端放 secret key。  
  憑證放在 `KnowledgeBit/SupabaseSecrets.swift`，`SupabaseConfig.swift` 僅轉接它。**此檔已列入 `.gitignore`，不應提交至版控**：請複製範本 `KnowledgeBit/SupabaseSecrets.example.swift` 為 `SupabaseSecrets.swift`，填入 `projectURL` 與 `publishableKey`，團隊以私下管道傳遞設定。  
  **本倉庫 git 歷史中曾包含舊版 legacy anon key**：僅從追蹤移除無法清除歷史中的舊值，請在 Dashboard **revoke（撤銷）該 legacy anon key**，改用新版 publishable key，這才能讓外洩的舊 key 失效。

- **Edge Functions（OpenAI）**  
  AI 產生單字卡、選擇題與講義教材所使用的 API Key，請僅在 Supabase 的 Edge Function **Secrets** 中設定 `OPENAI_API_KEY`（可選 `OPENAI_MODEL`，預設 `gpt-4o-mini`），勿寫入程式碼或前端。
