...
| **AI 產生單字卡** | 在新增單字時可輸入主題，呼叫 Supabase Edge Function，使用 **OpenAI（gpt-4o）** 一次產生多張單字卡（word / definition / example），並可排除單字集內已有單字，避免重複。 |
...
| AI 產生 | Supabase Edge Function + OpenAI API（gpt-4o，含 Responses API 處理講義 PDF） |
| 圖表 | Swift Charts（成就頁） |
| 版本控制 | Git / GitHub |
...
- **Edge Functions（OpenAI）**  
  AI 產生單字卡、選擇題與講義教材所使用的 API Key，請僅在 Supabase 的 Edge Function **Secrets** 中設定 `OPENAI_API_KEY`（可選 `OPENAI_MODEL`，預設 `gpt-4o`），勿寫入程式碼或前端。
